	...
	public class CaseController : Controller
	{
		...
		public async Task<IActionResult> Create()
		{
			var CaseVM = new CaseVM()
			{
				...
				Mediators = await _context.Mediators.AsNoTracking().ToArrayAsync(),
		...
		public async Task<IActionResult> Create(CaseVM model)
		{
			...
			model.Mediators = await _context.Mediators.AsNoTracking().ToArrayAsync();
			model.Relationships = await _context.Relationships.AsNoTracking().ToArrayAsync();
			model.Periods = await _context.Periods.AsNoTracking().ToArrayAsync();
			...
			var Case = new Case
			{
				...
				CategoryId = (byte)model.CaseCategoryId,
		...
		{
			var regions = await _context.Regions.AsNoTracking().Where(m => m.CityId == id).ToListAsync();
		...
		public async Task<IActionResult> Edit(CaseVM model)
		{
			...
			Case.CategoryId = (byte)model.CaseCategoryId;
		...
		//[HttpPost]
		//public async Task<IActionResult> ChangeStatus(uint id, byte statusId)
		//{
		...
		//	if (Status.Accepted.ToLower() == statusId)
		//	{
		//		@case.StatusId = 2;
